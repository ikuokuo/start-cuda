...
__global__ void what_is_my_id(unsigned int * const block,
    ...
    unsigned int * const wrap,
    unsigned int * const calc_thread) {
  // Thread id is block index * block szie + thread offset into the block
  const unsigned int thread_idx = (blockIdx.x * blockDim.x) + threadIdx.x;

  block[thread_idx] = blockIdx.x;
  thread[thread_idx] = threadIdx.x;

  // Calculate wrap using built in variable warpSize (线程束大小，32)
...
int main(void) {
  ...
  // Allocate foure arrays on the GPU
  ...
  what_is_my_id<<<num_blocks, num_threads>>>(
    gpu_block, gpu_thread, gpu_warp, gpu_calc_thread);
  ...
  // Iterate through the arrays and point
